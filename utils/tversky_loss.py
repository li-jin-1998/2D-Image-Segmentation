...
class TverskyLoss(nn.Module):
    def __init__(self, smooth=1e-6, alpha=0.7, beta=0.3, square=False):
        """
...
if __name__ == "__main__":
    y_pred = torch.rand((2, 3, 256, 256))  # 预测值，3个类别，256x256大小
    y_true = torch.randint(0, 2, (2, 3, 256, 256))  # 真实标签，二进制掩码

    # class_weights = torch.tensor([1.0, 2.0, 0.5])  # 类别权重
    focal_tversky = TverskyLoss()
    loss = focal_tversky(y_pred, y_true)
