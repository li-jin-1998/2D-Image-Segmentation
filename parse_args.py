...
def get_model(args):
    ...
    if args.arch == 'lraspp':
        ...
        model = lraspp_mobilenetv3_large(num_classes=args.num_classes, pretrain_backbone=True).to(device)
    if args.arch == 'u2net':
    ...
    if args.arch == 'stdcnet':
        ...
        model = BiSeNet('STDCNet813', n_classes=args.num_classes).to(device)
    ...
    if args.arch == 'bisenetv2':
        ...
        model = BiSeNetV2(n_classes=args.num_classes).to(device)
    if args.arch == 'resunet':
        from network.resunet import ResUNet
        model = ResUNet(num_classes=args.num_classes).to(device)
    if args.arch == 'liteseg':
        from network.liteseg.liteseg import liteSeg
        model = liteSeg(backbone_network="darknet", n_classes=args.num_classes).to(device)
    if args.arch == 'shufflenet':
        from network.shufflenet_unet import ShuffleUNet
        model = ShuffleUNet(num_classes=args.num_classes, pretrain_backbone=True).to(device)
...
def parse_args():
    ...
    parser.add_argument("--data_path", default="/mnt/algo_storage_server/UNet/Dataset11/data", help="root")
