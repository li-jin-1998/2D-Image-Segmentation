...
class Attention(nn.Module):
    def __init__(self, config, channel_num):
        ...
        self.query1 = nn.Linear(config.transformer["embedding_channels"], config.transformer["embedding_channels"],
                                bias=False)
        self.query2 = nn.Linear(config.transformer["embedding_channels"], config.transformer["embedding_channels"],
                                bias=False)
        self.query3 = nn.Linear(config.transformer["embedding_channels"], config.transformer["embedding_channels"],
                                bias=False)
        self.query4 = nn.Linear(config.transformer["embedding_channels"], config.transformer["embedding_channels"],
                                bias=False)
        self.key = nn.Linear(config.transformer["embedding_channels"], config.transformer["embedding_channels"],
                             bias=False)
        self.value = nn.Linear(config.transformer["embedding_channels"], config.transformer["embedding_channels"],
                               bias=False)
        ...
        self.out1 = nn.Linear(config.transformer["embedding_channels"], config.transformer["embedding_channels"],
                              bias=False)
        self.out2 = nn.Linear(config.transformer["embedding_channels"], config.transformer["embedding_channels"],
                              bias=False)
        self.out3 = nn.Linear(config.transformer["embedding_channels"], config.transformer["embedding_channels"],
                              bias=False)
        self.out4 = nn.Linear(config.transformer["embedding_channels"], config.transformer["embedding_channels"],
                              bias=False)
...
class DAT(nn.Module):
    def __init__(self, config, img_size, channel_num, patchSize):
        ...
        self.embeddings_2 = Spatial_Embeddings(config, self.patchSize_2, img_size=img_size // 2,
                                               in_channels=channel_num[1])
        self.embeddings_3 = Spatial_Embeddings(config, self.patchSize_3, img_size=img_size // 4,
                                               in_channels=channel_num[2])
        self.embeddings_4 = Spatial_Embeddings(config, self.patchSize_4, img_size=img_size // 8,
